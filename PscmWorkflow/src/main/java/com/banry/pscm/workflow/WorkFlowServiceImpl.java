...
public class WorkFlowServiceImpl implements WorkFlowService {
	...
	@Autowired
    protected SysUsersService userService;
  
    /* (non-Javadoc)
	 * @see com.banry.pscm.service.workflow.ProcessCoreService#startProcessInstanceByKey(java.lang.String, java.lang.String, java.util.Map)
	 */
	@Override
	@Transactional
	...
	/* (non-Javadoc)
	 * @see com.banry.pscm.service.workflow.WorkFlowService#complete(java.lang.String, java.util.Map)
	 */
	@Override
	@Transactional
	public void complete(String taskId, Map<String, Object> variables, String userCode) throws WorkFlowException {
		...
		if (task != null) {
			if (variables.containsKey("claim")) {
				taskService.addComment(taskId, task.getProcessInstanceId(), variables.get("claim").toString());
			}
	        // 判断任务是否已经被签收
			...
			throw new WorkFlowException("任务已经执行");
		}
	}
	
	/* (non-Javadoc)
	 * @see com.banry.pscm.service.workflow.WorkFlowService#getHisUserTaskActivityInstanceList(java.lang.String)
	 */
	@Override
	public List<ApprovalTrack> getHisUserTaskActivityInstanceList(String businessKey, String tenantCode, String parentTenantAccount) {
		// TODO Auto-generated method stub
		List<ApprovalTrack> trackList = new ArrayList<ApprovalTrack>();
...
//					.finished()
					...
					.list();
			for (HistoricActivityInstance entity : hisActivityInstanceList) {
				if ("userTask".equals(entity.getActivityType()) || "startEvent".equals(entity.getActivityType())
						|| "endEvent".equals(entity.getActivityType())) {
					ApprovalTrack track = new ApprovalTrack();
					track.setTrackcode(entity.getId());
					track.setBusinesskey(businessKey);
					track.setName(entity.getActivityName());
					track.setDealtime(entity.getEndTime());
					if ("userTask".equals(entity.getActivityType())) {
						String historytaskId = entity.getTaskId();
			            List<Comment> comments = taskService.getTaskComments(historytaskId);
			            if (comments!=null && comments.size()>0) {
			            	track.setClaim(comments.get(0).getFullMessage());
			            }
						try {
							if (entity.getAssignee() != null) {
								SysUsers u = userService.getByCodeAndTenantAccount(entity.getAssignee(), tenantCode, parentTenantAccount);
								if (u != null) {
					            	track.setDealername(u.getUserName());
					            }
							}
					...
					} else if ("startEvent".equals(entity.getActivityType())) {
						//上报人
						HistoricVariableInstance hv = historyService//
								.createHistoricVariableInstanceQuery() // 创建一个历史的流程变量查询对象
								.processInstanceId(processInstanceId)
								.variableName("reportUser").singleResult();
//						String reportUser = (String)runtimeService.getVariable(entity.getExecutionId(), "reportUser");
						if (hv != null) {
							track.setDealername(hv.getValue().toString());
						}
					}
					trackList.add(track);
				}
...
//             if (id == null || "".equals(id)) {// 并行起点ID为空，此节点流向不是并行终点，符合驳回条件，存储此节点  
